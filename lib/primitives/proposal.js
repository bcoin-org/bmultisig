...
class Proposal extends Struct {
  ...
  getProposalHash(type) {
    return sigUtils.getProposalHash(type, this.options);
  }
  ...
  verifySignature(type, signature, authPubKey) {
    assert((type & 0xff) === type, 'proposal type must be u8.');
    assert(Buffer.isBuffer(signature), 'signature must be a buffer.');
    assert(signature.length === 65, 'signature must be 65 bytes.');
    assert(Buffer.isBuffer(authPubKey), 'authPubKey must be a buffer.');
    assert(authPubKey.length === 33, 'authPubKey must be 33 bytes.');

    const hash = this.getProposalHash(type);
  ...
  verifyRejectSignature(signature, authPubKey) {
    return this.verifySignature(payloadType.REJECT, signature, authPubKey);
  }
  ...
  verifyCreateSignature(authPubKey) {
    return this.verifySignature(payloadType.CREATE, this.signature, authPubKey);
  }
