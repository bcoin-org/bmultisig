...
class Proposal extends Struct {
  ...
  fromOptions(options) {
    ...
    assert(typeof options.options === 'object',
  ...
  getJSON(tx, cosigners, network) {
    ...
    const approvals = this.approvals.toJSON();
    const rejections = this.rejections.toJSON();

    let author, cosignerApprovals, cosignerRejections;

    if (cosigners) {
      author = cosigners[this.author].toJSON(false, network);
      cosignerApprovals = [];
      cosignerRejections = [];

      for (const id of approvals)
        cosignerApprovals.push(cosigners[id].toJSON(false, network));

      for (const id of rejections)
        cosignerRejections.push(cosigners[id].toJSON(false, network));
    }
    ...
    return {
      ...
      cosignerApprovals: cosignerApprovals,
      cosignerRejections: cosignerRejections,
  ...
  toJSON(tx, cosigners, network) {
    return this.getJSON(tx, cosigners, network);
  }
  ...
  getProposalHash() {
    return sigUtils.getProposalHash(this.options);
  }

  /**
   * Verify proposal hash.
   * @param {Buffer} signature
   * @param {Buffer} authPubKey
   * @returns {Boolean}
   */

  verifyProposalSignature(signature, authPubKey) {
    assert(Buffer.isBuffer(signature), 'signature must be a buffer.');
    assert(signature.length === 65, 'signature must be 65 bytes.');
    assert(Buffer.isBuffer(authPubKey), 'authPubKey must be a buffer.');
    assert(authPubKey.length === 33, 'authPubKey must be 33 bytes.');

    const hash = this.getProposalHash();
  ...
  verifyAuthorProposalSignature(authPubKey) {
    assert(Buffer.isBuffer(authPubKey), 'authPubKey must be a buffer.');
    assert(authPubKey.length === 33, 'authPubKey must be 33 bytes.');

    const hash = this.getProposalHash();

    return sigUtils.verifyHash(hash, this.signature, authPubKey);
  }
