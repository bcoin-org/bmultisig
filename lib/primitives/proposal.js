...
class Proposal extends Struct {
  ...
  fromOptions(options) {
    ...
    assert((options.id >>> 0) === options.id, 'ID must be u32');
    ...
    assert((options.author & 0xff) === options.author, 'Author must be u8.');
    assert((options.n & 0xff) === options.n, 'n must be u8.');
    assert((options.m & 0xff) === options.m, 'm must be u8.');
