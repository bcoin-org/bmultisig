...
class Proposal extends Struct {
   ...
   * Get JSON
   * @param {TX?} tx
   * @param {Cosigners[]?} cosigners
   * @returns {Object}
   */

  getJSON(tx, cosigners) {
    let txhex = null;
    ...
    if (cosigners) {
      author = cosigners[this.author].toJSON();
      cosignerApprovals = [];
      cosignerRejections = [];

      for (const id of approvals)
        cosignerApprovals.push(cosigners[id].toJSON());

      for (const id of rejections)
        cosignerRejections.push(cosigners[id].toJSON());
    }
  ...
  toJSON(tx, cosigners) {
    return this.getJSON(tx, cosigners);
  }
