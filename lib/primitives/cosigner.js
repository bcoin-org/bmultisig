...
class Cosigner extends Struct {
  ...
  fromJSON(json, details, network) {
    ...
    assert(typeof json.key === 'object', 'Bad key.');
    ...
    const key = HDPublicKey.fromJSON(json.key);
  ...
  verifyProof(signature, network) {
    ...
    const hash = this.getProofHash(network);
  ...
  getProofHash(network) {
    return sigUtils.getProofHash(this, network);
  }
