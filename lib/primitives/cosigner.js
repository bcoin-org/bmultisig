...
class Cosigner extends Struct {
  ...
  fromOptions(options) {
    ...
    assert(Buffer.isBuffer(options.joinSignature), 'Bad joinSignature.');
    ...
    if (options.tokenDepth != null) {
      assert(isU32(options.tokenDepth), 'tokenDepth must be a uint32.');
    ...
    if (options.purpose != null) {
      assert(isU32(options.purpose), 'Purpose must be a uint32.');
      this.purpose = options.purpose;
    }

    if (options.fingerPrint != null) {
      assert(isU32(options.fingerPrint), 'fingerPrint must be a uint32.');
  ...
  /**
   * Clone cosigner object
   * @returns {Cosigner}
   */

  clone() {
    return new Cosigner().inject(this);
  }
  ...
  verifyProof(signature) {
    ...
    const proofKey = this.key.derive(sigUtils.PROOF_INDEX, 0);
    const hash = this.getProofHash();
  ...
  getProofHash() {
    return sigUtils.getProofHash(this);
  }
  ...
  verifyJoinSignature(joinPubKey, id) {
    ...
    const hash = this.getJoinHash(id);
  ...
  getJoinHash(id) {
    assert(typeof walletName === 'string', 'walletName must be a string.');

    return sigUtils.getJoinHash(id, this);
  }
