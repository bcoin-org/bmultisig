...
class ProposalDB {
  ...
  async createProposal(options, cosigner, mtx, signature) {
    ...
    try {
      return this._createProposal(options, cosigner, mtx, signature);
    } finally {
      unlock();
    }
  }
  ...
  async _createProposal(options, cosigner, mtx, signature) {
    ...
    if (!proposal.verifyCreateSignature(cosigner.authPubKey))
  ...
  async rejectProposal(id, cosigner, signature) {
    ...
    try {
      return this._rejectProposal(pid, cosigner, signature);
    } finally {
      unlock2();
      unlock1();
    }
