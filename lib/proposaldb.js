...
class ProposalDB {
  ...
  async forceRejectProposal(id) {
    ...
    try {
      return await this._forceRejectProposal(pid);
    } finally {
      unlock2();
      unlock1();
    }
  ...
  async _forceRejectProposal(pid) {
    const proposal = await this._getProposal(pid);
    ...
    proposal.forceReject(Proposal.status.FORCE);
    ...
    this.emit('proposal reject', proposal);
