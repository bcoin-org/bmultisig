...
sigutils.encodeProposalJSON = function encodeProposalJSON(type, json) {
  assert((type & 0xff) === type, 'type must be an u8.');
  assert(typeof json === 'string', 'JSON must be stringified.');

  const size = Buffer.byteLength(json, 'utf8') + 1;
  const data = Buffer.allocUnsafe(size);

  let written = 0;
  written += data.writeUInt8(type, 0);
  written += data.write(json, 1, 'utf8');
...
sigutils.getProposalHash = function getProposalHash(type, json) {
  const data = sigutils.encodeProposalJSON(type, json);
  return sigutils.hashMessage(data);
};
