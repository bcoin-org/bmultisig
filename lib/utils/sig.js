...
sigutils.encodeJoinMessage = function encodeJoinMessage(walletName, options) {
  ...
  bw.writeBytes(options.key.toRaw());
...
sigutils.getJoinHash = function getJoinHash(walletName, options) {
  const data = sigutils.encodeJoinMessage(walletName, options);
  return sigutils.hashMessage(data);
};
