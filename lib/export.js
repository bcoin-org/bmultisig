...
class AccountDetails extends Struct {
  ...
  read(br) {
    ...
    // defaults for multisig.
    // TODO: move this checks to multisigdb.
    // same checks apply whenever we receive these structures.
    assert(this.type === Account.types.MULTISIG);
    assert(this.name === 'default');
    assert(this.accountIndex === 0);
...
class WalletDetails extends Struct {
  ...
  write(bw, network) {
    ...
    bw.writeU8(this.id.length);
    bw.writeString(this.id, 'ascii');
  ...
  read(br, network) {
    ...
    const idlen = br.readU8();
    this.id = br.readString(idlen);
    this.timestamp = br.readU40();
    this.joinPubKey = br.readBytes(33);

    const accounts = br.readU8();
