/*!
 * http.js - http server for bmulsig
...
class HTTP extends Server {
  ...
  constructor(options) {
    ...
    this.logger = this.options.logger.context('mulsig-http');
...
class HTTPOptions {
  ...
  fromOptions(options) {
    assert(options, 'HTTP Server requires options');
    assert(options.node && options.node instanceof WalletNode,
      'HTTP Server requires WalletNode');
    ...
    if (options.logger != null) {
      assert(typeof options.logger === 'object',
        'HTTP Server requires correct logger'
      );
