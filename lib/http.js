...
class MultisigHTTP extends Server {
   ...
   * @param {blgr.Logger} options.logger
  ...
  initRouter() {
    ...
    this.put('/:id', async (req, res) => {
      ...
      const cosigner = Cosigner.fromOptions({
        ...
        authPubKey, joinSignature, token, key
      });
      ...
      enforce(validJoinSig, 'joinSignature is not valid.');

      // verify account key proof
      const validKeyProof = cosigner.verifyProof(accountKeyProof, this.network);
    ...
    this.post('/:id/join', async (req, res) => {
      ...
      // verify join signautre
      const validJoinSig = cosigner.verifyJoinSignature(
        req.mswallet.joinPubKey,
        req.mswallet.id,
        this.network
      );
    ...
    this.get('/:id/proposal', async (req, res) => {
      ...
      return res.json(200, {
        proposals: proposals.map((p) => {
          return p.toJSON(null, req.mswallet.cosigners, this.network);
        })
      });
    });
    ...
    this.post('/:id/proposal', async (req, res) => {
      const validReq = Validator.fromRequest(req);

      const proposalOptions = validReq.obj('proposal');
      const signature = validReq.buf('signature');

      const valid = new Validator(proposalOptions, false);
      const options = parseTXOptions(valid, this.network);
      ...
      res.json(200, proposal.toJSON(tx, req.mswallet.cosigners, this.network));
    });
    ...
    this.get('/:id/proposal/:pid', async (req, res) => {
      ...
      res.json(200, proposal.toJSON(null, cosigners, this.network));
    });
    ...
    this.post('/:id/proposal/:pid/approve', async (req, res) => {
      ...
      res.json(200, {
        broadcasted,
        proposal: proposal.toJSON(tx, req.wallet.cosigners, this.network)
      });
    });
  ...
  initSockets() {
    ...
    this.msdb.on('join', (wallet, cosigner) => {
      const json = cosigner.toJSON(false, this.network);

      handleEvent('join', wallet, json);
    });

    this.msdb.on('proposal created', (wallet, proposal, tx) => {
      const json = proposal.toJSON(tx, wallet.cosigners, this.network);
    ...
    this.msdb.on('proposal rejected', (wallet, proposal, cosigner) => {
      const json = {
        proposal: proposal.toJSON(null, wallet.cosigners, this.network),
        cosigner: cosigner ? cosigner.toJSON(false, this.network) : null
      };
    ...
    this.msdb.on('proposal approved', (wallet, proposal, cosigner, tx) => {
      const json = {
        proposal: proposal.toJSON(tx, wallet.cosigners, this.network),
        cosigner: cosigner.toJSON(false, this.network)
      };
