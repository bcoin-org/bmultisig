...
class MultisigHTTP extends Server {
  ...
  initRouter() {
    ...
    this.get('/:id/proposal/:pid', async (req, res) => {
      ...
      res.json(200, proposal.getJSON(null, cosigners, this.network));
    });
    ...
    this.post('/:id/proposal/:pid/approve', async (req, res) => {
      ...
      res.json(200, {
        ...
        proposal: proposal.getJSON(tx, req.mswallet.cosigners, this.network)
      });
    });
  ...
  initSockets() {
    ...
    this.msdb.on('proposal rejected', (wallet, proposal, cosigner) => {
      const json = {
        proposal: proposal.getJSON(null, wallet.cosigners, this.network),
    ...
    this.msdb.on('proposal approved', (wallet, proposal, cosigner, tx) => {
      const json = {
        proposal: proposal.getJSON(tx, wallet.cosigners, this.network),
