...
class MultisigHTTP extends Server {
  ...
  async cosignerAuth(req, res) {
    ...
    if (!cosignerToken || !mWallet)
      error(403, 'Authentication error.');

    const cosigner = mWallet.auth(cosignerToken);
  ...
  initRouter() {
    ...
    this.get('/:id', async (req, res) => {
      ...
      if (details)
        account = await req.mWallet.getAccount();

      res.json(200, req.mWallet.toJSON(false, balance, account));
    });
