 ...
 * @property {bcoin.wallet.WalletHTTP} whttp
 * @property {RouteList} notauthRoutes - skip authentication
 * @property {RouteList} proxyRoutes - proxy these to bcoin/wallet
 */
...
class MultisigHTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/:id/join', async (req, res) => {
      ...
      enforce(b58, 'accountKey is required');
      enforce(cosignerToken && cosignerToken.length === 32,
        'token needs to be 32 bytes.');
