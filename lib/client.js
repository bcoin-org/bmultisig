...
class MultisigClient extends WalletClient {
  ...
  getProposals(id, pending = true) {
    const proposalsObject = this.get(`/${id}/proposal`, { pending });
...
class MultisigWallet extends EventEmitter {
  ...
  getProposalTX(name, options) {
    return this.client.getProposalTX(this.id, name, options);
  }
