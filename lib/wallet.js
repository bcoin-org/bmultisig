...
class MultisigWallet extends EventEmitter {
  ...
  async createProposal(options, cosigner, txoptions) {
    const unlock = await this.coinLock.lock();

    try {
      return this._createProposal(options, cosigner, txoptions);
    } finally {
      unlock();
    }
  }
  ...
  async _createProposal(options, cosigner, txoptions) {
    const mtx = await this._createTX(txoptions);
    const proposal = await this.pdb.createProposal(options, cosigner, mtx);
