...
class MultisigWallet {
   ...
   * @async
   * @param {Object} options
   * @param {Number} options.rate - fee calculation ratejj
   ...
   * @param {Boolean} [includeTX=false]
   * @returns <Promise{Proposal?}>
   */

  async getProposal(id, includeTX) {
    let proposal;

    if (includeTX)
      proposal = await this.pdb.getWithTX(id);
    else
      proposal = await this.pdb.get(id);
